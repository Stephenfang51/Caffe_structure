//
// Created by StephenFang on 2019/11/9.
//
...
//dilation_h / dilation_w 空洞卷积 一般卷积时 为1即可
//data_col 输出矩阵


/// im2col_cpu具体实现 参考 https://blog.csdn.net/jiongnima/article/details/69736844
///以下代码主要讲输入的图像， 转化成矩阵， 一个通道一个通道进行变换， 为了方便执行矩阵相乘
//注意卷积中的zero-pad操作的实现，并不是真正在原始输入特征图周围添加0，而是在特征图转化得到的矩阵上的对应位置添加0
...
void im2col_cpu(const Dtype*data_im, const int channels,
    ...
    //计算output feat map的尺寸
    // output = H + 2*padd - (d(kernel_h - 1)+1)) / stride + 1
    ...
    for (int channel = channels; channel--; data_im+=channel_size) {
        ...
        for (int kernel_row = 0; kernel_row < kernel_h; kernel_row++){
            for (int kernel_col = 0; kernel_col < kernel_w; kernel_col++){
                int input_row = -pad_h + kernel_row * dilation_h;//计算出
                for (int output_rows = output_h; output_rows; output_rows--){
                    if(!is_a_ge_zero_and_a_lt_b(input_row, height)) //如果计算得到的输入图像的行值索引小于零或者大于输入图像的高(该行为pad)
                    {
                        ...
                        for (int output_col = output_w; output_col; output_col--) {
                            ...
                            input_col += stride_w; // 加上步长
                        }
                    }
                    input_row += stride_h;  // 加上步长
                }
            }
        }
    }
